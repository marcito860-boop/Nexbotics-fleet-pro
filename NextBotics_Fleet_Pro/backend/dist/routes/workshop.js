...
// Get all stock parts
router.get('/parts', auth_1.authenticateToken, (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
// Get single part
router.get('/parts/:id', auth_1.authenticateToken, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const part = await (0, database_1.query)(`
      SELECT sp.*, 
        COALESCE(SUM(su.quantity_used), 0) as total_used,
        COALESCE(SUM(su.quantity_used * su.unit_cost), 0) as total_cost
      FROM stock_parts sp
    ...
    // Get usage history
    const usage = await (0, database_1.query)(`
...
// Create new part
router.post('/parts', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'manager']), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { part_number, part_name, description, category, manufacturer, supplier, unit_cost, quantity_on_hand, reorder_level, location_bin, compatible_vehicles } = req.body;
    ...
    const result = await (0, database_1.query)('SELECT * FROM stock_parts WHERE id = $1', [id]);
    res.status(201).json(result[0]);
}));
// Update part
router.put('/parts/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'manager']), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    `, values);
    const result = await (0, database_1.query)('SELECT * FROM stock_parts WHERE id = $1', [req.params.id]);
    res.json(result[0]);
}));
// Adjust stock quantity (stock in/out)
router.post('/parts/:id/adjust', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'manager']), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    `, [(0, uuid_1.v4)(), req.params.id, adjustment, reason, req.user?.userId]);
    const result = await (0, database_1.query)('SELECT * FROM stock_parts WHERE id = $1', [req.params.id]);
    res.json(result[0]);
}));
// Record stock usage for a repair/job card
router.post('/usage', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'manager', 'transport_supervisor']), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const stock = await (0, database_1.query)('SELECT quantity_on_hand, unit_cost FROM stock_parts WHERE id = $1', [part_id]);
    if (stock.length === 0) {
        return res.status(404).json({ error: 'Part not found' });
    }
    ...
    `, [
        ...
        repair_id || null, job_card_id || null, notes, req.user?.userId
    ]);
    // Deduct from stock
    await (0, database_1.query)(`
      UPDATE stock_parts 
    ...
    const result = await (0, database_1.query)('SELECT * FROM stock_usage WHERE id = $1', [id]);
    res.status(201).json(result[0]);
}));
// Get stock usage report
router.get('/usage-report', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'manager']), (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
// ==================== INVOICING ====================
// Get all invoices
router.get('/invoices', auth_1.authenticateToken, (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
// Get single invoice
router.get('/invoices/:id', auth_1.authenticateToken, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const items = await (0, database_1.query)(`
...
router.post('/invoices/from-job-card', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'manager']), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const jobCard = await (0, database_1.query)(`
    ...
    const stockUsage = await (0, database_1.query)(`
    ...
    const countResult = await (0, database_1.query)("SELECT COUNT(*) as count FROM invoices WHERE EXTRACT(YEAR FROM invoice_date) = $1", [year]);
    const invoiceNumber = `INV-${year}-${String(parseInt(countResult[0].count) + 1).padStart(4, '0')}`;
    const invoiceId = (0, uuid_1.v4)();
    // Calculate totals
    const partsTotal = stockUsage.reduce((sum, item) => sum + (parseFloat(item.quantity_used) * parseFloat(item.unit_cost)), 0);
    ...
    for (const item of stockUsage) {
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [
            (0, uuid_1.v4)(), invoiceId, item.part_id, `${item.part_number} - ${item.part_name}`,
    ...
    if (laborTotal > 0) {
        ...
        VALUES ($1, $2, $3, $4, $5)
      `, [
            (0, uuid_1.v4)(), invoiceId, 'Labor - Repair Services',
            labor_hours || jobCard[0].actual_hours || 1, labor_rate || 50
        ]);
    }
    // Update job card
    await (0, database_1.query)('UPDATE job_cards SET invoice_id = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [invoiceId, job_card_id]);
    const result = await (0, database_1.query)('SELECT * FROM invoices WHERE id = $1', [invoiceId]);
    res.status(201).json(result[0]);
}));
// Update invoice status
router.patch('/invoices/:id/status', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'manager']), (0, errorHandler_1.asyncHandler)(async (req, res) => {
...
// Record payment
router.post('/invoices/:id/payments', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'manager']), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { amount, payment_method, reference, notes } = req.body;
    if (!amount || amount <= 0) {
        return res.status(400).json({ error: 'Valid payment amount required' });
    }
    const invoice = await (0, database_1.query)('SELECT total, amount_paid FROM invoices WHERE id = $1', [req.params.id]);
    if (invoice.length === 0) {
        return res.status(404).json({ error: 'Invoice not found' });
    }
    ...
    const result = await (0, database_1.query)('SELECT * FROM invoice_payments WHERE id = $1', [paymentId]);
    res.status(201).json(result[0]);
}));
// Get financial summary
router.get('/financial-summary', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'manager']), (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    // Invoice summary
    const invoiceSummary = await (0, database_1.query)(`
      SELECT 
    ...
    `);
    // Stock value
    const stockValue = await (0, database_1.query)(`
      SELECT 
