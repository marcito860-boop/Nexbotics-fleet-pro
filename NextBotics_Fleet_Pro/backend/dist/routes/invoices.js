...
router.get('/categories', async (req, res) => {
    try {
        ...
        const categories = await Invoice_1.InvoiceCategoryModel.findByCompany(companyId);
...
router.post('/categories', (0, auth_1.requireRole)(['admin', 'manager']), async (req, res) => {
    try {
        ...
        const category = await Invoice_1.InvoiceCategoryModel.create(companyId, name, description);
...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await Invoice_1.InvoiceModel.findByCompany(companyId, {
            status: status,
...
router.get('/stats', async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const stats = await Invoice_1.InvoiceModel.getStats(companyId);
...
router.get('/:id', async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const invoice = await Invoice_1.InvoiceModel.findById(req.params.id, companyId);
        if (!invoice) {
...
router.post('/', (0, auth_1.requireRole)(['admin', 'manager']), async (req, res) => {
    try {
        ...
        const invoice = await Invoice_1.InvoiceModel.create(companyId, userId, {
            invoiceNumber,
            categoryId,
            invoiceType,
            vendorName,
...
router.post('/:id/validate', (0, auth_1.requireRole)(['admin', 'manager']), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        const invoice = await Invoice_1.InvoiceModel.validate(req.params.id, companyId, userId);
        if (!invoice) {
...
router.post('/:id/approve', (0, auth_1.requireRole)(['admin', 'manager']), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        const invoice = await Invoice_1.InvoiceModel.approve(req.params.id, companyId, userId);
        if (!invoice) {
...
router.post('/:id/pay', (0, auth_1.requireRole)(['admin', 'manager']), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        ...
        const invoice = await Invoice_1.InvoiceModel.markPaid(req.params.id, companyId, userId, {
            paymentMethod,
...
router.post('/:id/cancel', (0, auth_1.requireRole)(['admin', 'manager']), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        const { reason } = req.body;
        const invoice = await Invoice_1.InvoiceModel.cancel(req.params.id, companyId, userId, reason);
        if (!invoice) {
