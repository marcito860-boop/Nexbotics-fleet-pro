...
const auth = (req, res, next) => {
    ...
    (0, auth_1.authenticateApiKey)(req, res, (err) => {
        if (err || !req.user) {
 ...
 *       - apiKeyAuth: []
 */
router.get('/vehicles', (0, errorHandler_1.asyncHandler)(async (req, res) => {
  ...
  `;
    ...
    const vehicles = await (0, database_1.query)(sql, params);
 ...
 *     tags: [Vehicles]
 */
router.get('/vehicles/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const companyId = req.user?.companyId;
    const userRole = req.user?.role;
 ...
 *     tags: [Drivers]
 */
router.get('/drivers', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const companyId = req.user?.companyId;
    const userRole = req.user?.role;
    let sql = `
    SELECT s.*, 
  ...
  `;
    ...
    const drivers = await (0, database_1.query)(sql, params);
 ...
 *     tags: [Drivers]
 */
router.get('/drivers/:id/performance', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const companyId = req.user?.companyId;
    // Get driver stats
    ...
    `, [id]),
        (0, database_1.query)(`SELECT COUNT(*) as count FROM accidents WHERE driver_id = $1`, [id]),
        (0, database_1.query)(`
 ...
 *     tags: [Inventory]
 */
router.get('/inventory', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const companyId = req.user?.companyId;
    const userRole = req.user?.role;
  ...
  `;
    ...
    if (low_stock === 'true' && items.length > 0) {
        await (0, webhook_1.emitWebhookEvent)('inventory.low_stock', {
            company_id: companyId,
            items: items.map((i) => ({ id: i.id, name: i.name, stock: i.current_stock })),
 ...
 *     tags: [Inventory]
 */
router.get('/inventory/categories', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const companyId = req.user?.companyId;
    const userRole = req.user?.role;
    ...
    sql += ` ORDER BY name`;
    const categories = await (0, database_1.query)(sql, params);
 ...
 *     tags: [Training]
 */
router.get('/courses', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const companyId = req.user?.companyId;
    const userRole = req.user?.role;
    ...
    const courses = await (0, database_1.query)(sql, params);
 ...
 *     tags: [Training]
 */
router.get('/courses/:id/enrollments', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
...
router.get('/alerts', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const alerts = await (0, database_1.query)(sql, params);
...
router.get('/invoices', (0, errorHandler_1.asyncHandler)(async (req, res) => {
  ...
  `;
    ...
    const invoices = await (0, database_1.query)(sql, params);
 ...
 *     tags: [Invoices]
 */
router.get('/invoices/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const [invoices, items] = await Promise.all([
        (0, database_1.query)(`SELECT * FROM invoices WHERE id = $1`, [id]),
        (0, database_1.query)(`SELECT * FROM invoice_items WHERE invoice_id = $1`, [id])
    ]);
 ...
 *     tags: [Maintenance]
 */
router.get('/maintenance', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const companyId = req.user?.companyId;
    const userRole = req.user?.role;
  ...
  `;
    ...
    const overdue = await (0, database_1.query)(overdueSql, params);
  ...
  `;
    ...
    const upcoming = await (0, database_1.query)(upcomingSql, params);
    // Emit webhook if maintenance is overdue
    if (overdue.length > 0) {
        await (0, webhook_1.emitWebhookEvent)('maintenance.overdue', {
            company_id: companyId,
            vehicles: overdue.map((v) => ({ id: v.id, registration: v.registration_num })),
...
router.get('/dashboard', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const [vehicles, drivers, pendingRepairs, lowStock, activeAlerts] = await Promise.all([
        (0, database_1.query)(`SELECT COUNT(*) as count FROM vehicles WHERE deleted_at IS NULL ${companyFilter}`, params),
        (0, database_1.query)(`SELECT COUNT(*) as count FROM staff WHERE role = 'Driver' AND deleted_at IS NULL ${companyFilter}`, params),
        (0, database_1.query)(`SELECT COUNT(*) as count FROM repairs WHERE status IN ('Pending', 'In Progress') ${companyFilter}`, params),
        (0, database_1.query)(`SELECT COUNT(*) as count FROM inventory_items WHERE current_stock <= reorder_level ${companyFilter}`, params),
        (0, database_1.query)(`SELECT COUNT(*) as count FROM risk_alerts WHERE resolved_at IS NULL ${companyFilter}`, params)
    ]);
    res.json({
        data: {
