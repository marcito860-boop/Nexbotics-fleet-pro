...
router.post('/schedules', async (req, res) => {
    const { template_id, schedule_name, frequency, day_of_week, day_of_month, start_date, end_date, auditor_id, branch } = req.body;
    try {
        // Calculate next run
...
router.put('/schedules/:id', async (req, res) => {
    const { id } = req.params;
    const { schedule_name, frequency, day_of_week, day_of_month, start_date, end_date, auditor_id, branch, is_active } = req.body;
    try {
        // Recalculate next run if frequency/start changed
        let nextRun = null;
        if (frequency || start_date !== undefined) {
            const current = await (0, database_1.query)('SELECT * FROM audit_schedules WHERE id = ?', [id]);
            if (current.length > 0) {
...
router.delete('/schedules/:id', async (req, res) => {
    const { id } = req.params;
    try {
        await (0, database_1.query)('DELETE FROM audit_schedules WHERE id = ?', [id]);
...
router.post('/scheduler/run', async (req, res) => {
    try {
        ...
        const dueSchedules = await (0, database_1.query)(`
    ...
    `);
        ...
        for (const schedule of dueSchedules) {
            try {
                ...
                const countResult = await (0, database_1.query)("SELECT COUNT(*) as count FROM audit_sessions WHERE EXTRACT(YEAR FROM created_at) = ?", [year]);
        ...
        `, [sessionId, auditNumber, schedule.template_id, schedule.auditor_id, schedule.branch]);
                ...
                const questions = await (0, database_1.query)('SELECT id FROM audit_questions WHERE template_id = ?', [schedule.template_id]);
                for (const q of questions) {
                    await (0, database_1.query)(`
            INSERT INTO audit_responses (id, session_id, question_id, response, score)
            VALUES (?, ?, ?, 'Not Checked', 0)
          `, [(0, uuid_1.v4)(), sessionId, q.id]);
                }
                // Record in history
                await (0, database_1.query)(`
          INSERT INTO audit_schedule_history (id, schedule_id, session_id)
          VALUES (?, ?, ?)
        `, [(0, uuid_1.v4)(), schedule.id, sessionId]);
...
//# sourceMappingURL=audit-schedules.js.map