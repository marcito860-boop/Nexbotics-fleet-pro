...
router.get('/audits', async (req, res) => {
    try {
        ...
        const reportData = await generateAuditReport(companyId, sessionId);
        if (format === 'json') {
...
router.get('/compliance', async (req, res) => {
    try {
        ...
        const [auditTrend, categoryScores, riskHeatmap] = await Promise.all([
         ...
         FROM risk_register
         WHERE company_id = $1
         GROUP BY category, risk_level`, [companyId]),
        ]);
        res.json({
            success: true,
            data: {
                auditTrend: auditTrend.map((r) => ({
                    month: r.month,
                    averageScore: parseFloat(r.avg_score),
                    count: parseInt(r.count),
                })),
                categoryScores: categoryScores.map((r) => ({
                    category: r.category,
                    averageScore: parseFloat(r.avg_score),
...
// HELPER FUNCTIONS
// ============================================
async function generateAuditReport(companyId, sessionId) {
    const [session, responses, categoryScores] = await Promise.all([
        (0, database_1.query)(`SELECT s.*, t.name as template_name, t.category,
       ...
       WHERE s.id = $1 AND s.company_id = $2`, [sessionId, companyId]),
        (0, database_1.query)(`SELECT r.*, q.question_text, q.description, q.category, q.question_number
       FROM audit_responses r
       JOIN audit_questions q ON r.question_id = q.id
       WHERE r.session_id = $1 AND r.company_id = $2
       ORDER BY q.question_number`, [sessionId, companyId]),
        (0, database_1.query)(`SELECT * FROM audit_category_scores WHERE session_id = $1 AND company_id = $2`, [sessionId, companyId]),
    ]);
    if (session.length === 0) {
