...
async function importVehicles(data) {
    const headers = data[0].map((h) => h.toLowerCase().trim());
    console.log('Vehicle headers:', headers);
    // Find column indices
    const getCol = (name) => headers.findIndex((h) => h.includes(name.toLowerCase()));
...
async function importRoutes(data) {
    const headers = data[0].map((h) => h.toLowerCase().trim());
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        try {
            ...
            if (driverRef) {
                const driverRes = await (0, database_1.query)('SELECT id FROM staff WHERE staff_no = $1 OR staff_name = $1', [driverRef]);
                if (driverRes.length > 0)
                    driverId = driverRes[0].id;
            }
            // Look up vehicle by registration_num
            let vehicleId = null;
            const vehicleRef = vehicleIdx >= 0 ? row[vehicleIdx] : '';
            if (vehicleRef) {
                const vehRes = await (0, database_1.query)('SELECT id FROM vehicles WHERE registration_num = $1', [vehicleRef]);
                if (vehRes.length > 0)
                    vehicleId = vehRes[0].id;
            }
            const id = (0, uuid_1.v4)();
...
async function importFuel(data) {
    const headers = data[0].map((h) => h.toLowerCase().trim());
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        try {
            // Lookup vehicle
            const vehicleRes = await (0, database_1.query)('SELECT id FROM vehicles WHERE registration_num = $1', [String(regNum).trim()]);
...
async function importRepairs(data) {
    const headers = data[0].map((h) => h.toLowerCase().trim());
...
async function importAccidents(data) {
    const headers = data[0].map((h) => h.toLowerCase().trim());
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        try {
            ...
            if (regRef) {
                const vehRes = await (0, database_1.query)('SELECT id FROM vehicles WHERE registration_num = $1', [regRef]);
                if (vehRes.length > 0)
                    vehicleId = vehRes[0].id;
            }
            ...
            const driverRef = driverIdx >= 0 ? row[driverIdx] : '';
            if (driverRef) {
                const drvRes = await (0, database_1.query)('SELECT id FROM staff WHERE staff_no = $1', [driverRef]);
                if (drvRes.length > 0)
                    driverId = drvRes[0].id;
            }
...
async function importRequisitions(data) {
    const headers = data[0].map((h) => h.toLowerCase().trim());
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        try {
            ...
            if (reqRef) {
                const reqRes = await (0, database_1.query)('SELECT id FROM staff WHERE staff_no = $1', [reqRef]);
                if (reqRes.length > 0)
                    requesterId = reqRes[0].id;
            }
            const id = (0, uuid_1.v4)();
