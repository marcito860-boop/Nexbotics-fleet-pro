...
function staffToDriver(staff) {
    ...
    if (staff.comments) {
        ...
        if (licenseMatch)
            licenseCategory = licenseMatch[1];
        if (expiryMatch)
...
router.post('/', async (req, res) => {
    ...
    if (isDriverFormat) {
        ...
        const { firstName, lastName, email, phone, licenseNumber, licenseCategory, licenseExpiry } = req.body;
        const companyId = req.user?.companyId;
      ...
      `, [id, staffNo, staffName, email, phone || 'N/A', 'Driver', 'Driver', `License: ${licenseCategory || 'B'}, Expiry: ${licenseExpiry || 'N/A'}`, companyId]);
            ...
            if (email && companyId && companyId !== 'super_admin') {
                try {
                    const existingUser = await User_1.UserModel.findByEmail(email, companyId);
                    if (!existingUser) {
                        const { user, tempPassword } = await User_1.UserModel.create({
                            email,
                            firstName,
                            lastName,
    ...
    const { staff_no, staff_name, email, phone, designation, department, branch, role, comments, create_user_account = true } = req.body;
    const companyId = req.user?.companyId;
    try {
        const id = (0, uuid_1.v4)();
        await (0, database_1.query)(`
...
router.post('/bulk-import', async (req, res) => {
    ...
    try {
        for (const staffData of staff_list) {
            const { staff_no, staff_name, email, phone, designation, department, branch, role, comments } = staffData;
            const detail = {
                staff_name,
                email,
                staffCreated: false,
            ...
            try {
                ...
                if (email) {
                    const existing = await (0, database_1.query)('SELECT id FROM staff WHERE (email = $1 OR staff_no = $2) AND company_id = $3', [email, staff_no, companyId]);
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        const staffResult = await (0, database_1.query)('SELECT email FROM staff WHERE id = $1', [id]);
        const staffEmail = staffResult[0]?.email;
        // Delete staff
        await (0, database_1.query)('DELETE FROM staff WHERE id = $1', [id]);
        // Optionally delete associated user account
        if (delete_user_account && staffEmail && companyId) {
            try {
                const user = await User_1.UserModel.findByEmail(staffEmail, companyId);
                if (user) {
                    await User_1.UserModel.delete(user.id, companyId, true);
                }
            }
            catch (userError) {
