...
router.post('/', async (req, res) => {
    const { fuel_date, vehicle_id, card_num, card_name, past_mileage, current_mileage, quantity_liters, amount, place } = req.body;
    ...
    try {
        ...
        if (companyId && companyId !== 'super_admin') {
            const vehicleCheck = await (0, database_1.query)('SELECT company_id FROM vehicles WHERE id = $1', [vehicle_id]);
...
router.put('/:id', async (req, res) => {
    const { id } = req.params;
    const { fuel_date, card_num, card_name, past_mileage, current_mileage, quantity_liters, amount, place } = req.body;
    try {
...
router.get('/cards', async (req, res) => {
    try {
        ...
        const tableCheck = await (0, database_1.query)(`
      SELECT EXISTS (
    ...
    `);
      ...
      `);
        ...
        try {
            await (0, database_1.query)(`ALTER TABLE fuel_cards ADD COLUMN IF NOT EXISTS company_id UUID REFERENCES companies(id) ON DELETE CASCADE`);
            await (0, database_1.query)(`ALTER TABLE fuel_cards ADD COLUMN IF NOT EXISTS card_num VARCHAR(100)`);
            await (0, database_1.query)(`ALTER TABLE fuel_cards ADD COLUMN IF NOT EXISTS card_number VARCHAR(100)`);
            await (0, database_1.query)(`ALTER TABLE fuel_cards ADD COLUMN IF NOT EXISTS card_name VARCHAR(255)`);
            await (0, database_1.query)(`ALTER TABLE fuel_cards ADD COLUMN IF NOT EXISTS assigned_vehicle_id UUID REFERENCES vehicles(id)`);
            await (0, database_1.query)(`ALTER TABLE fuel_cards ADD COLUMN IF NOT EXISTS monthly_limit DECIMAL(10,2)`);
            await (0, database_1.query)(`ALTER TABLE fuel_cards ADD COLUMN IF NOT EXISTS current_month_usage DECIMAL(10,2) DEFAULT 0`);
            await (0, database_1.query)(`ALTER TABLE fuel_cards ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT 'active'`);
        }
        catch (e) {
...
router.post('/cards', async (req, res) => {
    ...
    `, [id, companyId, card_num, card_name || '', assigned_vehicle_id || null, monthly_limit || null]);
        const result = await (0, database_1.query)('SELECT * FROM fuel_cards WHERE id = $1', [id]);
        res.status(201).json(result[0]);
    }
    catch (error) {
...
router.get('/analytics', async (req, res) => {
    try {
        ...
        // Overall summary
        const summaryResult = await (0, database_1.query)(`
      SELECT 
    ...
    `, params);
        ...
        const vehicleResult = await (0, database_1.query)(`
      SELECT 
        v.registration_num,
        v.make_model,
    ...
    `, params);
        ...
        const alertsResult = await (0, database_1.query)(`
      SELECT 
        v.registration_num,
        v.id as vehicle_id,
        AVG(f.km_per_liter) as avg_efficiency,
