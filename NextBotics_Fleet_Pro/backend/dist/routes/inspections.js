...
function generateInspectionNumber(type) {
    const date = new Date();
...
router.get('/checklist/:type', auth_1.authenticateToken, async (req, res) => {
...
router.get('/', auth_1.authenticateToken, async (req, res) => {
    const { vehicle_id, driver_id, inspection_type, status, from_date, to_date, overall_result, page = '1', limit = '50' } = req.query;
    try {
        let queryStr = `
      SELECT 
        i.*,
...
router.get('/:id', auth_1.authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const inspection = await (0, database_1.query)(`
      SELECT 
        i.*,
    ...
    `, [id]);
        ...
        const results = await (0, database_1.query)(`
      SELECT 
        ir.*,
    ...
    `, [id]);
        // Get defects
        const defects = await (0, database_1.query)(`
      SELECT 
        d.*,
    ...
    `, [id]);
        // Get photos
        const photos = await (0, database_1.query)(`
...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    const { vehicle_id, driver_id, route_id, requisition_id, assignment_id, inspection_type, trip_purpose, estimated_distance_km, route_from, route_to, starting_odometer, scheduled_at, weather_condition, road_condition, temperature_celsius, notes } = req.body;
...
router.post('/:id/complete', auth_1.authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { overall_result, ending_odometer, defects_summary, next_inspection_due, notes } = req.body;
    try {
        // Calculate distance traveled
        const inspection = await (0, database_1.query)('SELECT starting_odometer FROM vehicle_inspections WHERE id = $1', [id]);
...
router.delete('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'manager']), async (req, res) => {
    const { id } = req.params;
    try {
        await (0, database_1.query)('DELETE FROM vehicle_inspections WHERE id = $1', [id]);
...
router.post('/:id/results', auth_1.authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { item_id, result_status, severity, notes, measured_value, expected_value, photo_urls } = req.body;
    try {
    ...
    `, [
      ...
      `, [severity, id]);
        ...
        const result = await (0, database_1.query)('SELECT * FROM inspection_results WHERE id = $1', [resultId]);
        res.status(201).json(result[0]);
    }
    catch (error) {
...
router.post('/:id/results/batch', auth_1.authenticateToken, async (req, res) => {
    const { id } = req.params;
...
// ============================================
// Add defect
router.post('/:id/defects', auth_1.authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { result_id, defect_description, defect_category, severity, component_area, position_on_vehicle, photo_urls } = req.body;
    try {
        const defectId = (0, uuid_1.v4)();
        await (0, database_1.query)(`
    ...
    `, [
        ...
        const result = await (0, database_1.query)('SELECT * FROM inspection_defects WHERE id = $1', [defectId]);
        res.status(201).json(result[0]);
    }
    catch (error) {
...
router.put('/defects/:defectId', auth_1.authenticateToken, async (req, res) => {
...
router.get('/dashboard/stats', auth_1.authenticateToken, async (req, res) => {
    ...
    `);
        // Defects by category
        const defectsByCategory = await (0, database_1.query)(`
      SELECT 
        c.category_name,
        c.category_code,
    ...
    `);
        ...
        const byVehicle = await (0, database_1.query)(`
      SELECT 
        v.registration_num,
        v.make_model,
...
router.get('/vehicle/:vehicleId/history', auth_1.authenticateToken, async (req, res) => {
    ...
    `, [vehicleId, limit]);
        ...
        const trends = await (0, database_1.query)(`
      SELECT 
    ...
    `, [vehicleId]);
        ...
        const commonDefects = await (0, database_1.query)(`
      SELECT 
        d.defect_category,
...
router.post('/from-requisition/:requisitionId', auth_1.authenticateToken, async (req, res) => {
    const { requisitionId } = req.params;
    const { inspection_type = 'pre_trip' } = req.body;
    try {
        // Get requisition details
        const requisition = await (0, database_1.query)(`
      SELECT 
        r.*,
        v.registration_num, v.make_model,
        d.staff_name as driver_name
...
router.post('/from-route/:routeId', auth_1.authenticateToken, async (req, res) => {
    const { routeId } = req.params;
    const { inspection_type = 'post_trip' } = req.body;
    try {
        // Get route details
        const route = await (0, database_1.query)(`
      SELECT 
        r.*,
        v.registration_num, v.make_model,
...
router.get('/my-pending/:driverId', auth_1.authenticateToken, async (req, res) => {
    const { driverId } = req.params;
    try {
        const inspections = await (0, database_1.query)(`
      SELECT 
        i.*,
        v.registration_num, v.make_model,
        r.route_name, r.route_date,
