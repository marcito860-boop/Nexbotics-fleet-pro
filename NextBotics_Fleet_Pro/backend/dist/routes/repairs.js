...
router.post('/', async (req, res) => {
    const { date_in, vehicle_id, preventative_maintenance, breakdown_description, odometer_reading, driver_id, assigned_technician, repairs_start_time, target_repair_hours, garage_name, cost } = req.body;
    const companyId = req.user?.companyId;
    try {
    ...
    `, [
      ...
      WHERE id = $1
    `, [vehicle_id]);
        const result = await (0, database_1.query)('SELECT * FROM repairs WHERE id = $1', [id]);
...
router.put('/:id/complete', async (req, res) => {
    ...
    try {
        ...
        if (companyId && companyId !== 'super_admin') {
            const repairCheck = await (0, database_1.query)(`
      ...
      `, [id, companyId]);
        ...
        const repairData = await (0, database_1.query)('SELECT target_repair_hours, vehicle_id FROM repairs WHERE id = $1', [id]);
...
router.post('/job-cards', async (req, res) => {
    const { vehicle_id, defect_description, repair_type, service_provider, priority = 'Medium', estimated_cost } = req.body;
...
router.post('/job-cards/:id/approve', async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        if (companyId && companyId !== 'super_admin') {
            const jcCheck = await (0, database_1.query)(`
...
router.post('/job-cards/:id/assign', async (req, res) => {
    const { id } = req.params;
...
router.post('/job-cards/:id/complete', async (req, res) => {
    const { id } = req.params;
    ...
    try {
      ...
      `, [id, companyId]);
        ...
        const jobCardResult = await (0, database_1.query)('SELECT * FROM job_cards WHERE id = $1', [id]);
...
router.post('/job-cards/:id/cancel', async (req, res) => {
    const { id } = req.params;
    ...
    `, [cancellation_reason, id]);
        ...
        const otherPending = await (0, database_1.query)(`
...
router.post('/job-cards/:id/convert-to-repair', async (req, res) => {
    const { id } = req.params;
    const companyId = req.user?.companyId;
    try {
        // Verify job card belongs to user's company
      ...
      `, [id, companyId]);
        ...
        const jobCardResult = await (0, database_1.query)(`
