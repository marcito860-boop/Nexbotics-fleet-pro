...
router.post('/seed-data', auth_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        for (const v of vehicles) {
            const existing = await (0, database_1.query)('SELECT id FROM vehicles WHERE plate_number = $1 AND company_id = $2', [v.plate, companyId]);
            if (existing.length === 0) {
                await (0, database_1.query)(`INSERT INTO vehicles (company_id, plate_number, make, model, year, type, status, fuel_type, current_mileage, created_at, updated_at)
