...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const results = await Promise.allSettled([
         ...
         FROM drivers WHERE company_id = $1`, [companyId]).catch(() => (0, database_1.query)(`SELECT COUNT(*) as total, COUNT(*) as active FROM staff 
         WHERE (role = 'Driver' OR designation LIKE '%driver%') AND company_id = $1`, [companyId])),
            // Requisition stats
            (0, database_1.query)(`SELECT 
          COUNT(*) as total,
          COUNT(CASE WHEN status = 'pending' THEN 1 END) as pending
         FROM requisitions WHERE requested_by IN (SELECT id FROM staff WHERE company_id = $1)`, [companyId]).catch(() => [{ total: 0, pending: 0 }]),
            // Fuel stats
            (0, database_1.query)(`SELECT 
         ...
         AND fuel_date >= $2 AND fuel_date <= $3`, [companyId, fromDate, toDate]).catch(() => [{ total_cost: 0, total_liters: 0, transaction_count: 0, avg_price: 0 }]),
        ]);
        const [vehicleStats, driverStats, requisitionStats, fuelStats] = results.map(r => r.status === 'fulfilled' ? r.value : [{ total: 0, available: 0, assigned: 0, maintenance: 0 }]);
        const dashboard = {
            summary: {
...
router.get('/fleet', async (req, res) => {
    try {
        ...
        const utilizationByType = await (0, database_1.query)(`SELECT 
        v.type,
       ...
       GROUP BY v.type`, [companyId, fromDate, toDate]);
        // Maintenance trends
        const maintenanceTrends = await (0, database_1.query)(`SELECT 
       ...
       ORDER BY month`, [companyId, fromDate, toDate]);
        res.json({
            success: true,
            data: {
                ...
                maintenanceTrends: maintenanceTrends.map((r) => ({
                    month: r.month,
...
router.get('/audits', async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const stats = await Audit_1.AuditSessionModel.getStats(companyId);
        const categoryScores = await (0, database_1.query)(`SELECT 
        category,
        AVG(score_percentage) as avg_score,
        COUNT(*) as audit_count
       FROM audit_category_scores
       WHERE company_id = $1
       GROUP BY category`, [companyId]);
        res.json({
            success: true,
            data: {
                ...stats,
                categoryPerformance: categoryScores.map((r) => ({
                    category: r.category,
...
router.get('/fuel', async (req, res) => {
    try {
        ...
        const byVehicle = await (0, database_1.query)(`SELECT 
       ...
       ORDER BY total_cost DESC`, [companyId, fromDate, toDate]);
        res.json({
            success: true,
            data: {
                ...stats,
...
router.get('/my-performance', async (req, res) => {
    try {
        ...
        const driverRows = await (0, database_1.query)('SELECT id FROM drivers WHERE user_id = $1 AND company_id = $2', [userId, companyId]);
        const driverId = driverRows.length > 0 ? driverRows[0].id : null;
        const [trips, requisitions, correctiveActions, training] = await Promise.all([
            // My trips
            (0, database_1.query)(`SELECT 
          COUNT(*) as trip_count,
         ...
         FROM trips 
         WHERE company_id = $1 AND driver_id = $2 AND start_time >= $3 AND start_time <= $4`, [companyId, driverId, fromDate, toDate]),
            // My requisitions
            (0, database_1.query)(`SELECT 
          COUNT(*) as total,
          COUNT(CASE WHEN status = 'completed' THEN 1 END) as completed
         FROM vehicle_requisitions 
         WHERE company_id = $1 AND requested_by = $2 AND created_at >= $3 AND created_at <= $4`, [companyId, userId, fromDate, toDate]),
            // My corrective actions
            (0, database_1.query)(`SELECT 
          COUNT(*) as total,
          COUNT(CASE WHEN status = 'completed' THEN 1 END) as completed
         FROM corrective_actions 
         WHERE company_id = $1 AND assigned_to = $2`, [companyId, userId]),
            // My training
            (0, database_1.query)(`SELECT 
...
router.get('/utilization', async (req, res) => {
    try {
       ...
       WHERE v.company_id = $1
       GROUP BY v.type`, [companyId, fromDate, toDate]);
        const fleetRows = await (0, database_1.query)(`SELECT 
        COUNT(*) as total_trips,
        COALESCE(SUM(distance_km), 0) as total_distance,
        COALESCE(AVG(distance_km), 0) as avg_distance
       FROM trips 
       WHERE company_id = $1 AND start_time >= $2 AND start_time <= $3`, [companyId, fromDate, toDate]);
        res.json({
            success: true,
            data: {
...
router.get('/personal-performance', async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        const { dateFrom, dateTo } = req.query;
...
// HELPER FUNCTIONS
// ============================================
async function getHighRiskAreas(companyId) {
...
async function getComplianceRate(companyId) {
    const rows = await (0, database_1.query)(`SELECT 
