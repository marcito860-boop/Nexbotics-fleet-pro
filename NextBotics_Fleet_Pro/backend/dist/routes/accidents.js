...
const generateCaseNumber = async () => {
    ...
    const result = await (0, database_1.query)("SELECT COUNT(*) as count FROM accidents WHERE EXTRACT(YEAR FROM created_at) = $1", [year]);
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        // Get accident
        const accidentResult = await (0, database_1.query)(`
      SELECT a.*, 
        v.registration_num,
        d.staff_name as driver_name,
        r.route_name,
    ...
    `, [id]);
        ...
        // Get witnesses
        const witnesses = await (0, database_1.query)('SELECT * FROM accident_witnesses WHERE accident_id = $1', [id]);
        // Get evidence
        const evidence = await (0, database_1.query)('SELECT * FROM accident_evidence WHERE accident_id = $1', [id]);
        // Get investigation
        const investigationResult = await (0, database_1.query)(`
    ...
    `, [id]);
        // Get root cause
        const rootCauseResult = await (0, database_1.query)('SELECT * FROM accident_root_causes WHERE accident_id = $1', [id]);
        // Get CAPA
        const capa = await (0, database_1.query)(`
    ...
    `, [id]);
        // Get lessons
        const lessons = await (0, database_1.query)('SELECT * FROM accident_lessons WHERE accident_id = $1', [id]);
        res.json({
            ...accident,
            witnesses,
            evidence,
...
router.post('/', async (req, res) => {
    try {
        const { accident_date, gps_location, route_id, vehicle_id, driver_id, accident_type, severity, injuries_reported, police_notified, third_party_involved, weather_condition, road_condition, incident_description, witnesses } = req.body;
    ...
    `, [
            ...
            incident_description, req.user?.userId, 'Reported'
        ]);
        ...
        if (witnesses && witnesses.length > 0) {
          ...
          VALUES ($1, $2, $3, $4, $5)
        `, [(0, uuid_1.v4)(), id, witness.name, witness.contact, witness.statement]);
            }
        }
...
router.post('/:id/investigation', async (req, res) => {
    try {
        const { id } = req.params;
        const { investigator_id, investigation_date, scene_findings, vehicle_condition_assessment, driver_condition_assessment, valid_license, driver_training_compliant, speed_compliance, fatigue_status, alcohol_drug_test } = req.body;
    ...
    `, [
        ...
        await (0, database_1.query)("UPDATE accidents SET status = 'Under Investigation', updated_at = CURRENT_TIMESTAMP WHERE id = $1", [id]);
...
router.post('/:id/root-cause', async (req, res) => {
    try {
        const { id } = req.params;
        const { primary_category, primary_cause, contributing_factors, driver_causes, vehicle_causes, environmental_causes, organizational_causes, notes } = req.body;
        const rootCauseId = (0, uuid_1.v4)();
        await (0, database_1.query)(`
    ...
    `, [
        ...
        // Update accident status
        await (0, database_1.query)("UPDATE accidents SET status = 'Root Cause Identified', updated_at = CURRENT_TIMESTAMP WHERE id = $1", [id]);
...
router.post('/:id/capa', async (req, res) => {
    ...
    `, [capaId, id, action_description, responsible_person_id, target_completion_date, priority]);
        ...
        await (0, database_1.query)("UPDATE accidents SET status = 'CAPA In Progress', updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND status = 'Root Cause Identified'", [id]);
...
router.post('/:id/lessons', async (req, res) => {
    try {
        const { id } = req.params;
        const { key_lesson, preventive_recommendations, training_required, training_details, policy_update_needed, policy_update_details } = req.body;
...
router.post('/:id/close', async (req, res) => {
    try {
      ...
      SET status = 'Closed', 
          ...
          updated_at = CURRENT_TIMESTAMP
      WHERE id = $3
    `, [req.user?.id, closure_remarks, id]);
        res.json({ success: true });
    }
    catch (error) {
...
router.get('/analytics/dashboard', async (req, res) => {
    try {
        ...
        // Total accidents
        const totalResult = await (0, database_1.query)(`SELECT COUNT(*) as total FROM accidents ${dateFilter}`);
        // By severity
        const severityResult = await (0, database_1.query)(`
      SELECT severity, COUNT(*) as count FROM accidents ${dateFilter} GROUP BY severity
    `);
        // By type
        const typeResult = await (0, database_1.query)(`
    ...
    `);
        ...
        const driverResult = await (0, database_1.query)(`
...
const updateDriverSafetyScore = async (driverId) => {
    try {
        // Get accident count for driver
        const accidentResult = await (0, database_1.query)('SELECT COUNT(*) as count FROM accidents WHERE driver_id = $1', [driverId]);
        ...
        if (score < 60)
            ...
            rating = 'Good';
        await (0, database_1.query)('UPDATE staff SET safety_score = $1, safety_rating = $2 WHERE id = $3', [score, rating, driverId]);
    }
    catch (error) {
