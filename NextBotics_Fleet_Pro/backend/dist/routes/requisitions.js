...
router.get('/', async (req, res) => {
    try {
        ...
        const { status, requestedBy, dateFrom, dateTo, limit = '50', offset = '0' } = req.query;
        const result = await Requisition_1.RequisitionModel.findByCompany(companyId, {
            status: status,
...
router.get('/stats', async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const stats = await Requisition_1.RequisitionModel.getStats(companyId);
...
router.get('/:id', async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const requisition = await Requisition_1.RequisitionModel.findById(req.params.id, companyId);
        if (!requisition) {
...
router.get('/:id/history', async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const history = await Requisition_1.RequisitionModel.getWorkflowHistory(req.params.id, companyId);
...
router.post('/', async (req, res) => {
    try {
        ...
        const userId = req.user.userId;
        const user = req.user;
        const { requesterName, requesterDepartment, requesterPhone, purpose, destination, passengersCount, startDate, endDate, preferredVehicleType, requiresDriver, priority, } = req.body;
        ...
        const requisition = await Requisition_1.RequisitionModel.create(companyId, {
            requestedBy: userId,
...
router.post('/:id/approve', (0, auth_1.requireRole)(['admin', 'manager']), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        const { notes } = req.body;
        const requisition = await Requisition_1.RequisitionModel.approve(req.params.id, companyId, userId, notes);
        if (!requisition) {
...
router.post('/:id/reject', (0, auth_1.requireRole)(['admin', 'manager']), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        ...
        const requisition = await Requisition_1.RequisitionModel.reject(req.params.id, companyId, userId, reason);
        if (!requisition) {
            return res.status(404).json({ success: false, error: 'Requisition not found or not pending' });
        }
        res.json({ success: true, data: requisition });
...
router.post('/:id/allocate', (0, auth_1.requireRole)(['admin', 'manager']), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.userId;
        ...
        const requisition = await Requisition_1.RequisitionModel.allocate(req.params.id, companyId, userId, vehicleId, driverId);
        if (!requisition) {
...
router.post('/:id/start', (0, auth_1.requireRole)(['admin', 'manager', 'staff']), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        ...
        const requisition = await Requisition_1.RequisitionModel.startTrip(req.params.id, companyId, odometer);
        if (!requisition) {
...
router.post('/:id/complete', (0, auth_1.requireRole)(['admin', 'manager', 'staff']), async (req, res) => {
    try {
        ...
        const requisition = await Requisition_1.RequisitionModel.completeTrip(req.params.id, companyId, odometer, remarks);
        if (!requisition) {
...
router.post('/:id/cancel', async (req, res) => {
    try {
        ...
        const userId = req.user.userId;
        const { reason } = req.body;
        const requisition = await Requisition_1.RequisitionModel.cancel(req.params.id, companyId, userId, reason);
        if (!requisition) {
