...
exports.transaction = exports.pool = exports.query = exports.runMigrations = exports.initDatabase = void 0;
...
const createIndexes = async (poolRef) => {
    if (!poolRef)
        return;
    const indexes = [
        // Vehicles indexes
    ...
    if (pool) {
        try {
            ...
            if (adminResult.rows.length === 0) {
                // Create new admin user with default company_id and first_name
                await pool.query('INSERT INTO users (id, email, password_hash, role, company_id, first_name) VALUES ($1, $2, $3, $4, $5, $6)', [(0, uuid_1.v4)(), 'admin@fleet.local', hashedPassword, 'admin', '00000000-0000-0000-0000-000000000001', 'Admin']);
                console.log('✅ Default admin user created: admin@fleet.local / admin123');
            }
            else {
                // Reset password to ensure it's correct
                await pool.query('UPDATE users SET password_hash = $1, updated_at = CURRENT_TIMESTAMP, company_id = COALESCE(company_id, $3), first_name = COALESCE(first_name, $4) WHERE email = $2', [hashedPassword, 'admin@fleet.local', '00000000-0000-0000-0000-000000000001', 'Admin']);
...
// Seed sample slides for courses
const seedSampleSlides = async (pool) => {
    ...
    for (const [courseCode, slides] of Object.entries(sampleSlides)) {
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [(0, uuid_1.v4)(), courseId, i + 1, slide.title, slide.content, slide.duration]);
        }
...
const createAuditTables = async () => {
  ...
  `);
    ...
    `, [(0, uuid_1.v4)(), 'G4S Fleet Standard Audit', 'Comprehensive fleet operations audit covering 8 core modules with 45 checkpoint questions', true]);
        ...
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [(0, uuid_1.v4)(), g4sId, q.module, q.text, q.order, q.evidence]);
        }
        // Insert DVIR template
        const dvirResult = await pool.query(`
      INSERT INTO audit_templates (id, template_name, description, is_active)
      VALUES ($1, $2, $3, $4) RETURNING id
    `, [(0, uuid_1.v4)(), 'Daily Vehicle Inspection (DVIR)', 'Pre-trip and post-trip vehicle inspection checklist for drivers', true]);
        ...
        INSERT INTO audit_questions (id, template_id, module_name, question_text, question_order, requires_evidence)
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [(0, uuid_1.v4)(), dvirId, q.module, q.text, q.order, true]);
        }
...
const seedQuestionsIfMissing = async (pool) => {
    // Check G4S template
    const g4sResult = await pool.query("SELECT id FROM audit_templates WHERE template_name = $1", ['G4S Fleet Standard Audit']);
    if (g4sResult.rows.length > 0) {
        const g4sId = g4sResult.rows[0].id;
        const questionCount = await pool.query('SELECT COUNT(*) as count FROM audit_questions WHERE template_id = $1', [g4sId]);
    ...
    // Check DVIR template
    const dvirResult = await pool.query("SELECT id FROM audit_templates WHERE template_name = $1", ['Daily Vehicle Inspection (DVIR)']);
    if (dvirResult.rows.length > 0) {
        const dvirId = dvirResult.rows[0].id;
        const questionCount = await pool.query('SELECT COUNT(*) as count FROM audit_questions WHERE template_id = $1', [dvirId]);
        if (parseInt(questionCount.rows[0].count) === 0) {
            const dvirQuestions = [
                { module: 'Brakes', text: 'Service brakes functioning properly', order: 1 },
                { module: 'Brakes', text: 'Parking brake holds vehicle', order: 2 },
...
// Run inspection module migration
const runInspectionMigration = async (pool) => {
