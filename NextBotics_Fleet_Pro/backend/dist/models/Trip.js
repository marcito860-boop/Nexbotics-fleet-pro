...
class TripModel {
    static async findById(id, companyId) {
        const rows = await (0, database_1.query)(`SELECT t.*, 
    ...
    static async create(companyId, input) {
        const rows = await (0, database_1.query)(`INSERT INTO trips (company_id, vehicle_id, driver_id, assignment_id, purpose, start_time, start_odometer)
    ...
    static async update(id, companyId, input) {
        ...
        const rows = await (0, database_1.query)(`UPDATE trips SET ${updates.join(', ')} WHERE id = $${params.length - 1} AND company_id = $${params.length} RETURNING *`, params);
        return rows.length > 0 ? mapRowToTrip(rows[0]) : null;
    }
    static async complete(id, companyId, data) {
        ...
        const rows = await (0, database_1.query)(`UPDATE trips 
       ...
       RETURNING *`, [endTime, data.endOdometer, data.distanceKm, durationMinutes, data.fuelConsumed || null, id, companyId]);
        return rows.length > 0 ? mapRowToTrip(rows[0]) : null;
    }
