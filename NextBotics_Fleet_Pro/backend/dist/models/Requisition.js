...
exports.RequisitionModel = void 0;
const database_1 = require("../database");
function mapRowToRequisition(row) {
    return {
        id: row.id,
...
class RequisitionModel {
    static async generateRequestNumber(companyId) {
        const date = new Date();
        const prefix = `REQ-${date.getFullYear()}${String(date.getMonth() + 1).padStart(2, '0')}`;
        const rows = await (0, database_1.query)(`SELECT COUNT(*) as count FROM vehicle_requisitions 
       WHERE company_id = $1 AND request_number LIKE $2`, [companyId, `${prefix}-%`]);
    ...
    static async create(companyId, data) {
        ...
        const rows = await (0, database_1.query)(`INSERT INTO vehicle_requisitions (
    ...
    static async allocate(id, companyId, allocatedBy, vehicleId, driverId) {
       ...
       RETURNING *`, [vehicleId, driverId, allocatedBy, id, companyId]);
        if (rows.length === 0)
            return null;
        // Update vehicle status to assigned
        await (0, database_1.query)("UPDATE vehicles SET status = 'assigned', updated_at = NOW() WHERE id = $1 AND company_id = $2", [vehicleId, companyId]);
        await this.logWorkflowHistory(companyId, id, 'allocated', allocatedBy, `Allocated vehicle ${vehicleId}`);
        return this.findById(id, companyId);
    }
    static async startTrip(id, companyId, odometer) {
        const rows = await (0, database_1.query)(`UPDATE vehicle_requisitions 
       SET status = 'in_progress', actual_start_time = NOW(), actual_start_odometer = $1
       WHERE id = $2 AND company_id = $3 AND status = 'allocated'
       RETURNING *`, [odometer, id, companyId]);
        if (rows.length === 0)
            return null;
        const req = mapRowToRequisition(rows[0]);
    ...
    static async completeTrip(id, companyId, odometer, remarks) {
        const rows = await (0, database_1.query)(`UPDATE vehicle_requisitions 
       ...
       RETURNING *`, [odometer, remarks, id, companyId]);
        if (rows.length === 0)
            return null;
        const req = mapRowToRequisition(rows[0]);
        // Return vehicle to available status
        if (req.allocatedVehicleId) {
            await (0, database_1.query)("UPDATE vehicles SET status = 'available', current_odometer = $1, updated_at = NOW() WHERE id = $2 AND company_id = $3", [odometer, req.allocatedVehicleId, companyId]);
        }
    ...
    static async cancel(id, companyId, cancelledBy, reason) {
       ...
       RETURNING *`, [id, companyId]);
        ...
        if (req.allocatedVehicleId && req.status !== 'pending') {
            await (0, database_1.query)("UPDATE vehicles SET status = 'available', updated_at = NOW() WHERE id = $1 AND company_id = $2", [req.allocatedVehicleId, companyId]);
        }
        await this.logWorkflowHistory(companyId, id, 'cancelled', cancelledBy, reason || 'Requisition cancelled');
        return this.findById(id, companyId);
    }
    static async getWorkflowHistory(requisitionId, companyId) {
       ...
       ORDER BY h.action_at ASC`, [requisitionId, companyId]);
        return rows.map((row) => ({
            id: row.id,
            companyId: row.company_id,
    ...
    static async logWorkflowHistory(companyId, requisitionId, action, actionBy, notes) {
        await (0, database_1.query)(`INSERT INTO requisition_workflow_history (company_id, requisition_id, action, action_by, notes)
       VALUES ($1, $2, $3, $4, $5)`, [companyId, requisitionId, action, actionBy, notes]);
    }
}
