 ...
 */
const generateFleetRecommendations = async () => {
    ...
    try {
      ...
      LIMIT 20
    `);
        const fuelData = await (0, database_1.query)(`
      SELECT 
...
exports.generateFleetRecommendations = generateFleetRecommendations;
/**
 * Fallback rule-based recommendations when AI is unavailable
 */
const getRuleBasedRecommendations = async () => {
    const recommendations = [];
    // Check for overdue services
    const overdue = await (0, database_1.query)(`
  ...
  `);
    ...
    const fuelAnomalies = await (0, database_1.query)(`
  ...
  `);
    ...
    const openJobs = await (0, database_1.query)(`
  ...
  `);
    ...
    const defective = await (0, database_1.query)(`
 ...
 */
const generateSlideNotes = async (slideTitle, content) => {
    if (!exports.AI_ENABLED) {
...
exports.generateSlideNotes = generateSlideNotes;
/**
 * Process natural language analytics queries
 */
const processAnalyticsQuery = async (queryText) => {
    if (!exports.AI_ENABLED) {
...
exports.processAnalyticsQuery = processAnalyticsQuery;
/**
 * Rule-based query processing fallback
 */
const processRuleBasedQuery = async (queryText) => {
    ...
    if (lowerQuery.includes('fuel') || lowerQuery.includes('consumption') || lowerQuery.includes('efficiency')) {
        const data = await (0, database_1.query)(`
      SELECT 
    ...
    `);
        return {
            query: queryText,
            chartType: 'bar',
            data: data.map((d) => ({ name: d.name, value: parseFloat(d.value).toFixed(2) })),
  ...
  `);
    return {
        query: queryText,
        chartType: 'bar',
        data: data.map((d) => ({ name: d.name, value: parseInt(d.value) })),
 ...
 */
const executeParsedQuery = async (parsed) => {
 ...
 */
const generateCorrectiveActions = async (accidentDescription, severity) => {
    if (!exports.AI_ENABLED) {
...
exports.generateCorrectiveActions = generateCorrectiveActions;
const getDefaultCorrectiveActions = (severity) => {
    const actions = [
 ...
 */
const analyzeDriverBehavior = async (driverId) => {
    const whereClause = driverId ? 'WHERE s.id = $1' : '';
    const params = driverId ? [driverId] : [];
    const drivers = await (0, database_1.query)(`
    SELECT 
      s.id,
      s.staff_name,
      s.staff_no,
  ...
  `, params);
    const analyzeDriver = (d) => {
        ...
        if (d.avg_fuel_efficiency > 0) {
            if (d.avg_fuel_efficiency >= 12)
                fuelScore = 100;
            else if (d.avg_fuel_efficiency >= 10)
                fuelScore = 90;
            else if (d.avg_fuel_efficiency >= 8)
                fuelScore = 75;
            else if (d.avg_fuel_efficiency >= 6)
                fuelScore = 50;
            else
                fuelScore = 25;
        ...
        // Reliability scoring
        if (d.route_completion_rate >= 95)
            reliabilityScore = 100;
        else if (d.route_completion_rate >= 80)
            reliabilityScore = 75;
        else if (d.route_completion_rate >= 60)
            reliabilityScore = 50;
        else
            reliabilityScore = 25;
        ...
        const overallScore = Math.round((fuelScore * 0.30) + (safetyScore * 0.40) + (reliabilityScore * 0.30));
        // Risk level
        let riskLevel = 'low';
        if (overallScore < 40)
            riskLevel = 'critical';
...
exports.analyzeDriverBehavior = analyzeDriverBehavior;
/**
...
const detectAnomalies = async () => {
    ...
    const fuelStats = await (0, database_1.query)(`
    SELECT 
      v.id as vehicle_id,
      v.registration_num,
  ...
  `);
    ...
    // 2. Cost anomalies
    const costStats = await (0, database_1.query)(`
    SELECT 
      v.id as vehicle_id,
      v.registration_num,
 ...
 */
const predictMaintenanceCosts = async (vehicleId) => {
...
exports.predictMaintenanceCosts = predictMaintenanceCosts;
/**
 * Process chatbot queries about fleet data
 */
const processChatQuery = async (messages) => {
    console.log('🤖 processChatQuery called, AI_ENABLED:', exports.AI_ENABLED);
    ...
    try {
        ...
        const [vehicleCount, driverCount, pendingRepairs, todayRoutes] = await Promise.all([
            (0, database_1.query)('SELECT COUNT(*) as count FROM vehicles WHERE status = $1', ['Active']),
            (0, database_1.query)("SELECT COUNT(*) as count FROM staff WHERE role = $1", ['Driver']),
            (0, database_1.query)("SELECT COUNT(*) as count FROM repairs WHERE status = $1", ['Pending']),
            (0, database_1.query)('SELECT COUNT(*) as count FROM routes WHERE route_date = CURRENT_DATE')
        ]);
        const fleetContext = `
Fleet Status:
...
const processRuleBasedChat = async (queryText) => {
    const lowerQuery = queryText.toLowerCase();
    ...
    if (lowerQuery.includes('maintenance') && lowerQuery.includes('vehicle')) {
        const result = await (0, database_1.query)("SELECT COUNT(*) as count FROM vehicles WHERE status = 'Maintenance' AND deleted_at IS NULL");
        const vehicles = await (0, database_1.query)("SELECT registration_num, current_mileage FROM vehicles WHERE status = 'Maintenance' AND deleted_at IS NULL LIMIT 5");
        const count = result[0]?.count || 0;
        if (count === 0)
            return 'No vehicles are currently under maintenance.';
    ...
    if (lowerQuery.includes('vehicle') && (lowerQuery.includes('how many') || lowerQuery.includes('count') || lowerQuery.includes('total'))) {
        const total = await (0, database_1.query)('SELECT COUNT(*) as count FROM vehicles WHERE deleted_at IS NULL');
        const active = await (0, database_1.query)("SELECT COUNT(*) as count FROM vehicles WHERE status = 'Active' AND deleted_at IS NULL");
        const maintenance = await (0, database_1.query)("SELECT COUNT(*) as count FROM vehicles WHERE status = 'Maintenance' AND deleted_at IS NULL");
    ...
    if (lowerQuery.includes('route') && (lowerQuery.includes('today') || lowerQuery.includes('schedule'))) {
        const result = await (0, database_1.query)('SELECT COUNT(*) as count FROM routes WHERE route_date = CURRENT_DATE');
        const routes = await (0, database_1.query)(`
    ...
    if (lowerQuery.includes('inspection') || lowerQuery.includes('audit')) {
        const pending = await (0, database_1.query)("SELECT COUNT(*) as count FROM audit_sessions WHERE status = 'Pending'");
        const completed = await (0, database_1.query)("SELECT COUNT(*) as count FROM audit_sessions WHERE status = 'Completed' AND DATE(updated_at) = CURRENT_DATE");
        return `Inspection Status:\n• Pending: ${pending[0]?.count || 0}\n• Completed today: ${completed[0]?.count || 0}`;
    }
    // Repairs
    if (lowerQuery.includes('repair') || lowerQuery.includes('breakdown')) {
        const pending = await (0, database_1.query)("SELECT COUNT(*) as count FROM repairs WHERE status = 'Pending'");
        const inProgress = await (0, database_1.query)("SELECT COUNT(*) as count FROM repairs WHERE status = 'In Progress'");
        const recent = await (0, database_1.query)(`
    ...
    if (lowerQuery.includes('fuel')) {
        const today = await (0, database_1.query)('SELECT COALESCE(SUM(amount), 0) as total FROM fuel_records WHERE fuel_date = CURRENT_DATE');
        const month = await (0, database_1.query)("SELECT COALESCE(SUM(amount), 0) as total FROM fuel_records WHERE fuel_date > CURRENT_DATE - INTERVAL '30 days'");
        const avgEfficiency = await (0, database_1.query)("SELECT AVG(km_per_liter) as avg FROM fuel_records WHERE fuel_date > CURRENT_DATE - INTERVAL '30 days' AND km_per_liter IS NOT NULL");
    ...
    if (lowerQuery.includes('driver')) {
        const total = await (0, database_1.query)("SELECT COUNT(*) as count FROM staff WHERE role = 'Driver' AND deleted_at IS NULL");
        const active = await (0, database_1.query)(`
    ...
    if (lowerQuery.includes('training') || lowerQuery.includes('course')) {
        const courses = await (0, database_1.query)('SELECT COUNT(*) as count FROM training_courses');
        const enrollments = await (0, database_1.query)("SELECT COUNT(*) as count FROM training_enrollments WHERE status = 'in_progress'");
...
const fetchFleetContext = async (queryText) => {
    ...
    if (lowerQuery.includes('driver') || lowerQuery.includes('operator') || lowerQuery.includes('staff')) {
        context.drivers = await (0, database_1.query)(`
    ...
    if (lowerQuery.includes('route') || lowerQuery.includes('trip') || lowerQuery.includes('delivery')) {
        ...
        context.routes = await (0, database_1.query)(`
    ...
    if (lowerQuery.includes('inspection') || lowerQuery.includes('audit') || lowerQuery.includes('check')) {
        context.inspections = await (0, database_1.query)(`
    ...
    if (lowerQuery.includes('maintenance') || lowerQuery.includes('repair') || lowerQuery.includes('service') || lowerQuery.includes('job card')) {
        context.maintenance = await (0, database_1.query)(`
    ...
    if (lowerQuery.includes('accident') || lowerQuery.includes('incident') || lowerQuery.includes('crash')) {
        context.accidents = await (0, database_1.query)(`
    ...
    if (lowerQuery.includes('training') || lowerQuery.includes('course') || lowerQuery.includes('certification')) {
        context.training = await (0, database_1.query)(`
 ...
 */
const processFleetCopilotQuery = async (userQuestion) => {
    ...
    try {
        ...
        if (fleetContext.vehicles?.length) {
            ...
            fleetContext.vehicles.slice(0, 15).forEach((v) => {
        ...
        if (fleetContext.inspections?.length) {
            ...
            fleetContext.inspections.slice(0, 10).forEach((i) => {
        ...
        if (fleetContext.maintenance?.length) {
            ...
            fleetContext.maintenance.slice(0, 10).forEach((m) => {
        ...
        if (fleetContext.accidents?.length) {
            ...
            fleetContext.accidents.slice(0, 10).forEach((a) => {
        ...
        if (fleetContext.training?.length) {
            ...
            fleetContext.training.forEach((t) => {
...
exports.processFleetCopilotQuery = processFleetCopilotQuery;
/**
 * Enhanced rule-based copilot with full context
 */
const processRuleBasedCopilot = async (queryText, context) => {
    const lowerQuery = queryText.toLowerCase();
    // Vehicles under maintenance
    if ((lowerQuery.includes('maintenance') || lowerQuery.includes('under repair')) && lowerQuery.includes('vehicle')) {
        const maintenanceVehicles = context.vehicles?.filter((v) => v.status === 'Maintenance') || [];
    ...
    if (lowerQuery.includes('overdue') && (lowerQuery.includes('inspection') || lowerQuery.includes('audit'))) {
        const overdue = context.inspections?.filter((i) => i.status === 'Pending' || i.status === 'Overdue') || [];
        if (overdue.length === 0) {
    ...
    if (lowerQuery.includes('delay') || lowerQuery.includes('late') || lowerQuery.includes('missed')) {
        const completedRoutes = context.routes?.filter((r) => r.actual_km && r.target_km) || [];
        const delayed = completedRoutes.filter((r) => r.actual_km < r.target_km * 0.8);
        if (delayed.length === 0) {
    ...
    if (lowerQuery.includes('pending') && (lowerQuery.includes('repair') || lowerQuery.includes('maintenance'))) {
        const pending = context.maintenance?.filter((m) => m.status !== 'Completed' && m.status !== 'Closed') || [];
        if (pending.length === 0) {
    ...
    if (lowerQuery.includes('how many') || lowerQuery.includes('count') || lowerQuery.includes('total')) {
        const active = context.vehicles?.filter((v) => v.status === 'Active').length || 0;
        const maintenance = context.vehicles?.filter((v) => v.status === 'Maintenance').length || 0;
    ...
    if (lowerQuery.includes('today') && lowerQuery.includes('route')) {
        const todayRoutes = context.routes?.filter((r) => r.route_date === new Date().toISOString().split('T')[0]) || [];
    ...
    if (lowerQuery.includes('accident') || lowerQuery.includes('incident')) {
        ...
        context.accidents.slice(0, 5).forEach((a) => {
...
// ==================== EXPORT ====================
exports.AI_ENABLED = !!process.env.OPENAI_API_KEY;
exports.default = {
    generateFleetRecommendations: exports.generateFleetRecommendations,
    generateSlideNotes: exports.generateSlideNotes,
    processAnalyticsQuery: exports.processAnalyticsQuery,
    generateCorrectiveActions: exports.generateCorrectiveActions,
    analyzeDriverBehavior: exports.analyzeDriverBehavior,
    detectAnomalies: exports.detectAnomalies,
    predictMaintenanceCosts: exports.predictMaintenanceCosts,
    processChatQuery: exports.processChatQuery,
    processFleetCopilotQuery: exports.processFleetCopilotQuery,
    AI_ENABLED: exports.AI_ENABLED
};
/**
 * Calculate vehicle risk score based on multiple factors
 */
const calculateVehicleRisk = async (vehicleId) => {
...
exports.calculateVehicleRisk = calculateVehicleRisk;
/**
...
const generateRiskAlerts = async () => {
    ...
    const repeatedDefects = await (0, database_1.query)(`
    SELECT 
      v.id,
      v.registration_num,
  ...
  `);
    ...
    const riskyDrivers = await (0, database_1.query)(`
    SELECT 
      s.id,
      s.staff_name,
  ...
  `);
    ...
    const overdueAudits = await (0, database_1.query)(`
    SELECT 
      v.id,
      v.registration_num,
  ...
  `);
    ...
    const maintenanceDue = await (0, database_1.query)(`
    SELECT 
      v.id,
      v.registration_num,
      v.next_service_due,
  ...
  `);
    ...
    const highRepairRate = await (0, database_1.query)(`
    SELECT 
      v.id,
      v.registration_num,
...
exports.generateRiskAlerts = generateRiskAlerts;
/**
 * Get fleet intelligence summary for dashboard
 */
const getFleetIntelligenceSummary = async () => {
    const riskProfiles = await (0, exports.calculateVehicleRisk)();
    const alerts = await (0, exports.generateRiskAlerts)();
    ...
    HAVING MAX(audit.audit_date) IS NULL OR MAX(audit.audit_date) < CURRENT_DATE - INTERVAL '90 days'
  `);
    const maintenanceDueSoon = await (0, database_1.query)(`
  ...
  `);
    const driverSafetyAlerts = await (0, database_1.query)(`
...
exports.getFleetIntelligenceSummary = getFleetIntelligenceSummary;
/**
 * Get predictive maintenance suggestions
 */
const getPredictiveMaintenanceSuggestions = async () => {
    const suggestions = [];
    // Analyze defect trends
    const defectTrends = await (0, database_1.query)(`
    SELECT 
      v.id,
      v.registration_num,
  ...
  `);
    ...
    const accidentPatterns = await (0, database_1.query)(`
    SELECT 
      v.id,
      v.registration_num,
...
exports.getPredictiveMaintenanceSuggestions = getPredictiveMaintenanceSuggestions;
/**
 * Generate training quiz questions based on course content
 */
const generateTrainingQuestions = async (content, numQuestions = 5) => {
...
[
  {
    ...
    "correctAnswer": "A"
        ...
        return questions.filter((q) => q.question &&
            ...
            q.options.length === 4 &&
            ['A', 'B', 'C', 'D'].includes(q.correctAnswer)).slice(0, numQuestions);
    }
    catch (error) {
...
exports.generateTrainingQuestions = generateTrainingQuestions;
/**
 * Fallback rule-based questions when AI is unavailable
 */
const getRuleBasedQuestions = (content, numQuestions) => {
    const questions = [];
