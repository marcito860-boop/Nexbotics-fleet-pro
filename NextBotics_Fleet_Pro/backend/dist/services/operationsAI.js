...
exports.broadcastOperationsUpdate = exports.getLiveFleetStatus = exports.optimizeRoute = exports.getFleetHealthSummary = exports.analyzeVehicleHealth = void 0;
...
exports.analyzeVehicleHealth = analyzeVehicleHealth;
// Get fleet-wide health summary
const getFleetHealthSummary = async () => {
    const allVehicles = await (0, exports.analyzeVehicleHealth)();
...
exports.getFleetHealthSummary = getFleetHealthSummary;
...
const optimizeRoute = async (request) => {
    ...
    if (endLocation) {
        totalDistance += calculateDistance(current.lat, current.lng, endLocation.lat, endLocation.lng);
    }
...
const getLiveFleetStatus = async () => {
    const today = new Date().toISOString().split('T')[0];
    // Today's routes
    const todaysRoutes = await (0, database_1.query)(`
    SELECT 
      r.id,
      r.route_name,
      r.route_date,
  ...
  `);
    ...
    const activeVehicles = todaysRoutes.filter((r) => r.status === 'In Progress' || r.status === 'Completed').length;
    // Available vehicles
    const availableVehicles = await (0, database_1.query)(`
  ...
  `);
    ...
    const pendingRequisitions = await (0, database_1.query)(`
    SELECT COUNT(*) as count 
    FROM requisitions 
    WHERE status = 'pending'
  `);
    // Critical alerts
    const criticalAlerts = await (0, database_1.query)(`
    SELECT 
  ...
  `);
    // Recent accidents
    const recentAccidents = await (0, database_1.query)(`
    SELECT 
      a.id,
      a.case_number,
      a.accident_date,
...
exports.getLiveFleetStatus = getLiveFleetStatus;
// Broadcast live updates via WebSocket
const broadcastOperationsUpdate = async () => {
    if (!index_1.io)
        return;
    const status = await (0, exports.getLiveFleetStatus)();
    const health = await (0, exports.getFleetHealthSummary)();
    index_1.io.emit('operations:update', {
        timestamp: new Date().toISOString(),
        status,
        health
    });
};
exports.broadcastOperationsUpdate = broadcastOperationsUpdate;
//# sourceMappingURL=operationsAI.js.map