...
// ==================== Mapping Functions ====================
function mapRowToServiceProvider(row: any): ServiceProvider {
  return {
    id: row.id,
...
function mapRowToSparePart(row: any): SparePart {
  return {
    id: row.id,
    companyId: row.company_id,
...
function mapRowToSchedule(row: any): MaintenanceSchedule {
  return {
    id: row.id,
    companyId: row.company_id,
...
function mapRowToRecord(row: any): MaintenanceRecord {
  return {
    id: row.id,
    companyId: row.company_id,
    vehicleId: row.vehicle_id,
...
function mapRowToMaintenancePart(row: any): MaintenancePart {
  return {
    id: row.id,
...
function mapRowToDowntime(row: any): VehicleDowntime {
  return {
    id: row.id,
    companyId: row.company_id,
    vehicleId: row.vehicle_id,
...
function mapRowToReminder(row: any): MaintenanceReminder {
  return {
    id: row.id,
    companyId: row.company_id,
    scheduleId: row.schedule_id,
...
export class ServiceProviderModel {
  static async findById(id: string, companyId: string): Promise<ServiceProvider | null> {
    const rows = await query(
      'SELECT * FROM service_providers WHERE id = $1 AND company_id = $2',
      [id, companyId]
    );
  ...
  static async create(companyId: string, input: CreateServiceProviderInput): Promise<ServiceProvider> {
    const rows = await query(
  ...
  static async update(id: string, companyId: string, input: Partial<CreateServiceProviderInput>): Promise<ServiceProvider | null> {
    ...
    const rows = await query(
      `UPDATE service_providers SET ${updates.join(', ')} WHERE id = $${params.length - 1} AND company_id = $${params.length} RETURNING *`,
      params
    );
  ...
  static async delete(id: string, companyId: string): Promise<boolean> {
    const result = await query(
      'DELETE FROM service_providers WHERE id = $1 AND company_id = $2 RETURNING id',
      [id, companyId]
    );
...
export class SparePartModel {
  static async findById(id: string, companyId: string): Promise<SparePart | null> {
    const rows = await query(
      'SELECT * FROM spare_parts WHERE id = $1 AND company_id = $2',
      [id, companyId]
    );
    return rows.length > 0 ? mapRowToSparePart(rows[0]) : null;
  }

  static async findByPartNumber(partNumber: string, companyId: string): Promise<SparePart | null> {
    const rows = await query(
      'SELECT * FROM spare_parts WHERE part_number = $1 AND company_id = $2',
      [partNumber.toUpperCase(), companyId]
    );
    return rows.length > 0 ? mapRowToSparePart(rows[0]) : null;
  }
  ...
  static async create(companyId: string, input: CreateSparePartInput): Promise<SparePart> {
    const rows = await query(
  ...
  static async update(id: string, companyId: string, input: Partial<CreateSparePartInput>): Promise<SparePart | null> {
    const updates: string[] = [];
    const params: any[] = [];
    ...
    params.push(id, companyId);
    const rows = await query(
      `UPDATE spare_parts SET ${updates.join(', ')} WHERE id = $${params.length - 1} AND company_id = $${params.length} RETURNING *`,
      params
    );
  ...
  static async adjustStock(id: string, companyId: string, quantity: number, reason?: string): Promise<SparePart | null> {
    const rows = await query(
      `UPDATE spare_parts 
       SET quantity_in_stock = quantity_in_stock + $1, updated_at = CURRENT_TIMESTAMP 
       WHERE id = $2 AND company_id = $3 
       RETURNING *`,
      [quantity, id, companyId]
    );
    return rows.length > 0 ? mapRowToSparePart(rows[0]) : null;
  }
  ...
  static async delete(id: string, companyId: string): Promise<boolean> {
    const result = await query(
      'DELETE FROM spare_parts WHERE id = $1 AND company_id = $2 RETURNING id',
      [id, companyId]
    );
    return result.length > 0;
  }
...
export class MaintenanceScheduleModel {
  static async findById(id: string, companyId: string): Promise<MaintenanceSchedule | null> {
    const rows = await query(
  ...
  static async create(companyId: string, input: CreateScheduleInput): Promise<MaintenanceSchedule> {
  ...
  static async update(id: string, companyId: string, input: Partial<CreateScheduleInput> & { status?: ScheduleStatus }): Promise<MaintenanceSchedule | null> {
    const updates: string[] = [];
    const params: any[] = [];
    ...
    params.push(id, companyId);
    const rows = await query(
      `UPDATE maintenance_schedules SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP 
  ...
  static async delete(id: string, companyId: string): Promise<boolean> {
    const result = await query(
      'DELETE FROM maintenance_schedules WHERE id = $1 AND company_id = $2 RETURNING id',
      [id, companyId]
    );
    return result.length > 0;
  }
  ...
  static async getStats(companyId: string): Promise<{
    ...
    const rows = await query(
      `SELECT 
       ...
       FROM maintenance_schedules WHERE company_id = $1`,
      [companyId]
    );
...
export class MaintenanceRecordModel {
  static async findById(id: string, companyId: string): Promise<MaintenanceRecord | null> {
    const rows = await query(
  ...
  static async getParts(recordId: string): Promise<MaintenancePart[]> {
    const rows = await query(
      'SELECT * FROM maintenance_parts WHERE record_id = $1',
      [recordId]
    );
    ...
    try {
      ...
      if (input.parts && input.parts.length > 0) {
        for (const part of input.parts) {
          await client.query(
            `INSERT INTO maintenance_parts (record_id, part_id, part_number, part_name, quantity, unit_cost)
             VALUES ($1, $2, $3, $4, $5, $6)`,
            [record.id, part.partId || null, part.partNumber, part.partName, part.quantity, part.unitCost]
          );
      ...
      if (input.status === 'completed' && input.serviceMileage) {
        await client.query(
          'UPDATE vehicles SET current_mileage = $1, last_service_date = $2 WHERE id = $3',
          [input.serviceMileage, input.completedDate || new Date(), input.vehicleId]
        );
      }
  ...
  static async update(id: string, companyId: string, input: Partial<CreateRecordInput>): Promise<MaintenanceRecord | null> {
    const updates: string[] = [];
    const params: any[] = [];
    ...
    params.push(id, companyId);
    const rows = await query(
      `UPDATE maintenance_records SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP 
       WHERE id = $${params.length - 1} AND company_id = $${params.length} 
       RETURNING *`,
      params
    );
  ...
  static async delete(id: string, companyId: string): Promise<boolean> {
    const result = await query(
      'DELETE FROM maintenance_records WHERE id = $1 AND company_id = $2 RETURNING id',
      [id, companyId]
    );
    return result.length > 0;
  }
...
export class VehicleDowntimeModel {
  static async findById(id: string, companyId: string): Promise<VehicleDowntime | null> {
    const rows = await query(
  ...
  static async findByCompany(companyId: string, options?: {
    ...
    limit?: number;
    offset?: number;
  }): Promise<{ downtimes: VehicleDowntime[]; total: number }> {
    let sql = `
      SELECT vd.*, v.registration_number as vehicle_registration,
             rv.registration_number as replacement_vehicle_registration
      FROM vehicle_downtime vd
  ...
  static async create(companyId: string, input: CreateDowntimeInput): Promise<VehicleDowntime> {
    const rows = await query(
  ...
  static async endDowntime(id: string, companyId: string, endDate: Date, endTime?: string, durationHours?: number): Promise<VehicleDowntime | null> {
    const rows = await query(
      `UPDATE vehicle_downtime 
       SET end_date = $1, end_time = $2, duration_hours = $3, updated_at = CURRENT_TIMESTAMP 
       WHERE id = $4 AND company_id = $5 
       RETURNING *`,
      [endDate, endTime || null, durationHours || null, id, companyId]
    );
...
export class MaintenanceReminderModel {
  ...
  static async create(companyId: string, scheduleId: string, vehicleId: string, input: {
    reminderType: string;
    title: string;
    message?: string;
    dueMileage?: number;
  ...
  static async acknowledge(id: string, companyId: string, userId: string): Promise<MaintenanceReminder | null> {
    const rows = await query(
  ...
  static async dismiss(id: string, companyId: string): Promise<MaintenanceReminder | null> {
    const rows = await query(
      `UPDATE maintenance_reminders 
  ...
  static async generateFromSchedules(companyId: string): Promise<number> {
    // Find schedules that need reminders and create them
    const result = await query(
      `INSERT INTO maintenance_reminders (company_id, schedule_id, vehicle_id, reminder_type, title, message, due_mileage, due_date, severity)
       SELECT 
         ms.company_id,
  ...
  static async getStats(companyId: string): Promise<{
    ...
    const rows = await query(
      `SELECT 
       ...
       FROM maintenance_reminders WHERE company_id = $1`,
      [companyId]
    );
