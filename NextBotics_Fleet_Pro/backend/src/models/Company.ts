...
import { generateCompanySlug } from '../utils/password';
...
export class CompanyModel {
  ...
  static async findBySlug(slug: string): Promise<Company | null> {
    const rows = await query('SELECT * FROM companies WHERE slug = $1', [slug.toLowerCase()]);
    return rows.length > 0 ? mapRowToCompany(rows[0]) : null;
  }
  ...
  static async findByEmail(email: string): Promise<Company | null> {
    ...
    const rows = await query(
      'SELECT * FROM companies WHERE email LIKE $1 LIMIT 1',
      [`%@${domain}`]
    );
    return rows.length > 0 ? mapRowToCompany(rows[0]) : null;
  }
  ...
  static async create(input: CreateCompanyInput): Promise<Company> {
    const slug = input.slug || generateCompanySlug(input.name);
  ...
  // Update company
  static async update(id: string, updates: Partial<CreateCompanyInput>): Promise<Company | null> {
    ...
    const rows = await query(
      `UPDATE companies SET ${setClauses.join(', ')} WHERE id = $${params.length} RETURNING *`,
      params
    );
  ...
  static async updateSubscription(
    ...
    const rows = await query(
      'UPDATE companies SET subscription_plan = $1, subscription_status = $2 WHERE id = $3 RETURNING *',
      [plan, status, id]
    );
    return rows.length > 0 ? mapRowToCompany(rows[0]) : null;
  }
  ...
  // Update settings
  static async updateSettings(id: string, settings: Record<string, any>): Promise<Company | null> {
    const rows = await query(
      'UPDATE companies SET settings = settings || $1 WHERE id = $2 RETURNING *',
