...
function mapRowToAlert(row: any): FleetAlert {
  return {
    id: row.id,
...
export class AlertModel {
  static async findById(id: string, companyId: string): Promise<FleetAlert | null> {
  ...
  static async findByCompany(companyId: string, options?: {
    ...
    limit?: number;
    offset?: number;
  }): Promise<{ alerts: FleetAlert[]; total: number; unreadCount: number }> {
    let sql = `
      SELECT a.*, 
        v.registration_number as v_reg,
        d.first_name as d_fname, d.last_name as d_lname
  ...
  static async create(companyId: string, input: CreateAlertInput): Promise<FleetAlert> {
    const rows = await query(
  ...
  static async markAsRead(id: string, companyId: string, userId: string): Promise<void> {
    await query(
      `UPDATE fleet_alerts 
  ...
  static async markAllAsRead(companyId: string, userId: string): Promise<void> {
    await query(
      `UPDATE fleet_alerts 
       SET is_read = true, read_at = CURRENT_TIMESTAMP, read_by = $1
       WHERE company_id = $2 AND is_read = false`,
      [userId, companyId]
    );
  }

  static async dismiss(id: string, companyId: string, userId: string): Promise<void> {
    await query(
      `UPDATE fleet_alerts 
  ...
  static async delete(id: string, companyId: string): Promise<boolean> {
    const result = await query(
      'DELETE FROM fleet_alerts WHERE id = $1 AND company_id = $2 RETURNING id',
      [id, companyId]
    );
    return result.length > 0;
  }

  static async deleteOldAlerts(companyId: string, days: number): Promise<number> {
    const result = await query(
  ...
  static async generateMaintenanceAlerts(companyId: string): Promise<void> {
    // Find vehicles with maintenance due
    const vehicles = await query(
      `SELECT id, registration_number, next_service_due, current_mileage, service_interval_km
       FROM vehicles 
       WHERE company_id = $1 
       AND is_active = true
       AND (
    ...
    for (const vehicle of vehicles) {
      ...
      const existing = await query(
        `SELECT id FROM fleet_alerts 
         WHERE company_id = $1 AND vehicle_id = $2 AND alert_type = 'maintenance_due' AND is_read = false`,
  ...
  static async generateExpiryAlerts(companyId: string): Promise<void> {
    // Insurance expiry alerts
    const insuranceExpiring = await query(
    ...
    for (const vehicle of insuranceExpiring) {
      const existing = await query(
        `SELECT id FROM fleet_alerts 
         WHERE company_id = $1 AND vehicle_id = $2 AND alert_type = 'insurance_expiry' AND is_read = false`,
        [companyId, vehicle.id]
      );
    ...
    const licenseExpiring = await query(
      `SELECT id, registration_number, license_expiry
       FROM vehicles 
       WHERE company_id = $1 
       AND is_active = true
    ...
    for (const vehicle of licenseExpiring) {
      const existing = await query(
        `SELECT id FROM fleet_alerts 
         WHERE company_id = $1 AND vehicle_id = $2 AND alert_type = 'license_expiry' AND is_read = false`,
        [companyId, vehicle.id]
      );
