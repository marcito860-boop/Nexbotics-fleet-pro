  ...
  if (pool) {
    try {
      ...
      if (adminResult.rows.length === 0) {
        ...
        await pool.query(
          'UPDATE users SET password_hash = $1, updated_at = CURRENT_TIMESTAMP, company_id = COALESCE(company_id, $3), first_name = COALESCE(first_name, $4), last_name = COALESCE(last_name, $5) WHERE email = $2',
          [hashedPassword, 'admin@fleet.local', '00000000-0000-0000-0000-000000000001', 'Admin', 'User']
        );
