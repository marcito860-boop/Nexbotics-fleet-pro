  ...
  if (pool) {
    try {
      ...
      if (adminResult.rows.length === 0) {
        ...
        await pool.query(
          'UPDATE users SET password_hash = $1, updated_at = CURRENT_TIMESTAMP, first_name = COALESCE(first_name, $3), last_name = COALESCE(last_name, $4) WHERE email = $2',
          [hashedPassword, 'admin@fleet.local', 'Admin', 'User']
        );
