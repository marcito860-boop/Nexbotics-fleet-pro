  ...
  if (pool) {
    try {
      ...
      if (adminResult.rows.length === 0) {
        ...
        await pool.query(
          'INSERT INTO users (id, email, password_hash, role, company_id, first_name) VALUES ($1, $2, $3, $4, $5, $6)',
          [uuidv4(), 'admin@fleet.local', hashedPassword, 'admin', '00000000-0000-0000-0000-000000000001', 'Admin']
        );
        console.log('✅ Default admin user created: admin@fleet.local / admin123');
      } else {
        // Reset password to ensure it's correct
        await pool.query(
          'UPDATE users SET password_hash = $1, updated_at = CURRENT_TIMESTAMP, company_id = COALESCE(company_id, $3), first_name = COALESCE(first_name, $4) WHERE email = $2',
          [hashedPassword, 'admin@fleet.local', '00000000-0000-0000-0000-000000000001', 'Admin']
        );
